...
export function middleware(request: NextRequest) {
  ...
  // Handle auth routes - always allow access, let client-side handle redirects
  // This prevents issues with expired tokens blocking access to login page
  if (isAuthRoute(pathname)) {
    // Always allow access to auth routes
    // Client-side components will handle redirecting authenticated users
    return NextResponse.next()
  }
